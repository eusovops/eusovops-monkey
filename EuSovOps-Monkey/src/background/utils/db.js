...
export async function parseScript(src) {
  ...
  // Check for duplicate script names (regardless of namespace)
  const duplicateByName = aliveScripts.find(({ meta: m, props: p }) =>
    m.name === meta.name && p.id !== id
  );
  if (duplicateByName && !oldScript) {
    throw `A script with the name "${meta.name}" already exists. Please use a different name.`;
  }
