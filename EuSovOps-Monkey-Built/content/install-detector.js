// EuSovOps-Monkey Install Detector
...
(function() {
  ...
  function showInstallBanner() {
    ...
    if (document.getElementById('eusovops-install-banner')) return;

    const banner = document.createElement('div');
    banner.id = 'eusovops-install-banner';
    ...
    banner.innerHTML = `
      <div style="display: flex; align-items: center; gap: 16px;">
        <div style="font-size: 18px; font-weight: 500;">EuSovOps-Monkey</div>
        <div>Do you want to install this userscript?</div>
      </div>
      <div style="display: flex; gap: 8px;">
        <button id="eusovops-install-btn" style="
          background: #4caf50;
          color: white;
          border: none;
          padding: 8px 24px;
          ...
          font-weight: 500;
        ">Install</button>
        <button id="eusovops-close-btn" style="
    ...
    // Add event listeners
    document.getElementById('eusovops-install-btn').addEventListener('click', () => {
      installScript();
    });

    document.getElementById('eusovops-close-btn').addEventListener('click', () => {
      banner.remove();
    });
  }
  ...
  function installScript() {
    ...
    chrome.runtime.sendMessage({
      ...
      if (response && response.success) {
        ...
        const banner = document.getElementById('eusovops-install-banner');
